...
@requires(lambda lit, ret: forall(lambda v: int(v) > 0,
                                  select_all(xpath(Lit, "..integer"), lit)))
def all_int_pos(lit: Lit) -> int:
    return 0


@requires(lambda lit, ret: forall(lambda v: int(v) > 0,
                                  select_all(xpath(Lit, "..list.value.integer"), lit)))
def all_int_elem_pos(lit: Lit) -> int:
    return 0


@requires(lambda lit, ret: forall(lambda v: int(v) > 0,
                                  select_all(xpath(Lit, ".value.list.value.integer"), lit)))
def list_all_elem_pos(lit: Lit) -> int:
    return 0


@requires(lambda lit, ret: forall(lambda v: int(v) > 0,
                                  select_all(xpath(Lit, ".value.list.value[1]..integer"), lit)))
def list_first_elem_all_int_pos(lit: Lit) -> int:
    return 0


@requires(lambda lit, ret: forall(lambda v: int(v) > 0,
                                  select_all(xpath(Lit, ".value.list.value[1]..list.value.integer"), lit)))
